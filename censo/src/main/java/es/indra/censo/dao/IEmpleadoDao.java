...
public interface IEmpleadoDao extends PagingAndSortingRepository<Empleado, Integer> {
	...
	@Query(value= "SELECT e FROM Empleado e WHERE e.nombre LIKE CONCAT('%',:name,'%') AND "
			+ "e.apellido LIKE  CONCAT('%',:surname,'%')",
			countQuery = "SELECT COUNT(*) FROM Empleado e WHERE e.nombre LIKE CONCAT('%',:name,'%') AND "
					+ "e.apellido LIKE  CONCAT('%',:surname,'%')")
	public Page<Empleado> findEmpleadoByNameAndSurname(@Param("name") String name,
